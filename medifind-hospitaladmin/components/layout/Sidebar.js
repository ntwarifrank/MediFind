...
const Sidebar = ({ isMobileOpen = false, onClose }) => {
  ...
  const navItems = [
    ...
    // Hospital Management
    { name: 'Services', path: '/services', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div 
        className={`fixed h-screen bg-gradient-to-b from-deepBlue to-mainBlue text-white shadow-lg z-40 transition-all duration-300 w-[280px] md:w-64 overflow-hidden ${isMobileOpen ? 'left-0' : '-left-[280px] md:left-0'}`}
      >
        {/* Mobile close button */}
        ...
        <div className="p-4 md:p-6">
          <h1 className="text-xl md:text-2xl font-bold">MediFind</h1>
          <p className="text-sm opacity-75">Hospital Admin</p>
        </div>
        
        {/* Navigation */}
        <nav className="mt-4 md:mt-6 overflow-y-auto h-[calc(100vh-240px)] custom-scrollbar">
          <ul className="px-2">
            {navItems.map((item) => {
              const isActive = pathname === item.path;
              return (
                <li key={item.name} className="mb-1 md:mb-2">
                  <Link href={item.path}>
                    <div
                      className={`flex items-center px-4 md:px-6 py-2 md:py-3 text-sm transition-colors duration-200 ${isActive
        ...
        <div className="px-4 py-4 border-t border-white/10 mt-4">
          <div className="text-xs text-white/70 mb-2">Quick Links</div>
          <div className="space-y-2">
            <Link href="/help" className="flex items-center px-2 py-1.5 text-sm text-white/80 hover:text-white transition-colors">
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              Help Center
            </Link>
            <Link href="/privacy" className="flex items-center px-2 py-1.5 text-sm text-white/80 hover:text-white transition-colors">
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* User profile */}
        <div className="fixed bottom-0 w-full md:w-64 p-4 md:p-6 border-t border-white/10 bg-gradient-to-r from-deepBlue to-mainBlue">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-xs text-white/60">{user?.role || 'Hospital Admin'}</p>
              </div>
            </div>
            <button 
              onClick={logout}
