...
export const getAllDoctors = async (req, res) => {
  try {
    ...
    if (req.query.hospitalId || req.params.hospitalId) {
...
export const deleteDoctor = async (req, res) => {
  try {
    ...
    if (isHospitalAdmin) {
      ...
      // Check permissions - different properties might be used in different places
      if ((hospital.adminName && hospital.adminName.toString() !== req.user._id.toString()) ||
          (hospital.admin && hospital.admin.toString() !== req.user._id.toString())) {
        return res.status(403).json({
          status: 'fail',
      ...
      return res.status(403).json({
        status: 'fail',
        message: 'You do not have permission to delete doctor information'
      });
    }
...
export const getDoctorSchedule = async (req, res) => {
  try {
    ...
    const doctor = await Doctor.findById(req.params.id).select('name availableDays');
